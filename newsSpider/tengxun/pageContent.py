...
class pageContent:
    ...
    def getWxContent(self,wxUrl):#  先提取出里面的那个，然后再看看什么情况  ----这个是主要解析的函数
        ...
        if(title!=None and title.text!="404 - 腾讯网"):
            title=title.text
            if self.dbhelper.ifExists(title):  #如果标题为空那就直接删除这条了。
...
if __name__ == "__main__":  #这个就是url的东西,再统一化后，就确实和框架差不多，只有爬取部分需要自自定义
    ...
    #这儿开始测试新的代码
    urlResult = tengxun.changeNewToWx("http:////auto.qq.com//a//20190117//009589.htm")   #一个视频的页面，还是有问题哈 https://news.qq.com//a//20181004//007938.htm
