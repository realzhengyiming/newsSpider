...
def isRunning(process_name):
    try:   #默认这样就输出三个输出
        '''
        ps aux | grep  /root/newsSpider/tempApplication/multiprocesses.py | grep -v grep   #这样的话就刚好4个，1个主进程，3个子进程。
        ps aux | grep  /root/newsSpider/tempApplication/multiprocesses.py | grep -v grep   #这样的话就刚好4个，1个主进程，3个子进程。
        '''

        process = len(os.popen('ps aux | grep "' + "test .py" + '" | grep -v grep').readlines())
        print("多少个数量")    #
        for i in  os.popen('ps aux | grep "' + "test.py" + '" | grep -v grep').readlines():
            print(i)
